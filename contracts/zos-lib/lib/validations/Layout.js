  ...
  for (let i = 1; i <= a.length; i++) {
    for (let j = 1; j <= b.length; j++) {
      if (areEqualFn(a[i - 1], b[j - 1])) {
        ...
        matrix[i][j] = Math.min(matrix[i - 1][j - 1] + SUBSTITUTION_COST, matrix[i][j - 1] + insertionCost, matrix[i - 1][j] + DELETION_COST);
      }
    }
  }
...
function walk(matrix, originalStorage, updatedStorage, areMatchFn) {
  ...
  while (i > 0 || j > 0) {
    ...
    const updated = j > 0 && _extends({ index: j - 1 }, b[j - 1]);
    const original = i > 0 && _extends({ index: i - 1 }, a[i - 1]);
