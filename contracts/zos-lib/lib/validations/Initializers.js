 ...
 */
function getUninitializedBaseContracts(contractClass) {
  const uninitializedBaseContracts = {};
  getUninitializedDirectBaseContracts(contractClass, uninitializedBaseContracts);
  return _lodash2.default.invertBy(uninitializedBaseContracts);
}
...
function getUninitializedDirectBaseContracts(contractClass, uninitializedBaseContracts) {
  ...
  for (const baseContract of baseContracts) {
    ...
    const baseContractClass = _Contracts2.default.getFromLocal(baseContractName);
...
function getContractInitializer(contractClass) {
  const contractDefinition = contractClass.ast.nodes.find(n => n.nodeType === "ContractDefinition" && n.name === contractClass.contractName);
