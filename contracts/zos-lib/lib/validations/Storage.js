...
function getStorageLayout(contract, artifacts) {
  if (!artifacts) artifacts = (0, _BuildArtifacts.getBuildArtifacts)();
  const layout = new StorageLayout(contract, artifacts);
  const { types, storage } = layout.run();
  return { types, storage };
}

function getStructsOrEnums({ storage, types }) {
...
class StorageLayout {
  ...
  visitVariables(contractNode) {
    const sourcePath = _path2.default.relative(_process2.default.cwd(), this.getNode(contractNode.scope, 'SourceUnit').absolutePath);
    ...
    varNodes.forEach(node => {
      ...
      const storageInfo = _extends({ contract: contractNode.name, path: sourcePath }, this.getStorageInfo(node, typeInfo));
  ...
  getLinearizedBaseContracts() {
    return _lodash2.default.reverse(this.getContractNode().linearizedBaseContracts.map(id => this.getNode(id, 'ContractDefinition')));
  }
  ...
  getTypeInfo(node) {
    switch (node.nodeType) {
      ...
      case 'FunctionTypeName':
        return this.getFunctionTypeInfo(node);
      default:
  ...
  getUserDefinedTypeInfo({ referencedDeclaration, typeDescriptions }) {
    ...
    switch (typeIdentifier) {
      case 't_contract':
        return this.getContractTypeInfo(referencedDeclaration);
      case 't_struct':
  ...
  getStructTypeInfo(referencedDeclaration) {
    ...
    const members = referencedNode.members.filter(member => member.nodeType === 'VariableDeclaration').map(member => {
