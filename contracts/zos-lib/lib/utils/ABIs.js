...
var _ContractAST = require('./ContractAST');

var _ContractAST2 = _interopRequireDefault(_ContractAST);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function buildCallData(contractClass, methodName, args) {
  const method = getABIFunction(contractClass, methodName, args);
  const argTypes = method.inputs.map(input => input.type);
  const callData = (0, _encodeCall2.default)(method.name, argTypes, args);
  return { method, callData };
}

function getABIFunction(contractClass, methodName, args) {
  ...
  switch (abiMethods.length) {
    ...
    case 1:
      return abiMethods[0];
    default:
      throw Error(`Found more than one match for function ${methodName} with ${args.length} arguments in contract ${contractClass.contractName}`);
  }
}
...
function tryGetFunctionNodeFromMostDerivedContract(contractClass, methodName, args) {
  ...
  const nodeMatches = node => node.nodeType === 'FunctionDefinition' && node.name === methodName && node.parameters.parameters.length === args.length;
...
function tryGetLinearizedBaseContracts(contractClass) {
  try {
    const ast = new _ContractAST2.default(contractClass, null, { nodesFilter: ['ContractDefinition', 'FunctionDefinition'] });
